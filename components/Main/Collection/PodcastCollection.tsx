...
export const PodcastCollection: React.FC<PodcastCollectionProps> = ({}) => {
  const { data: session } = useSession();
  const spotifyAPI = useSpotify();
  const router = useRouter();
  const [colour, setColour] = useState<any>(null);
  const [usersPodcasts, setUsersPodcasts] = useState<any[]>([]);
  const {
  ...
  useEffect(() => {
    if (libraryCollectionSelected === "podcasts") {
      spotifyAPI
        .getMySavedShows()
        .then((data) => {
          setUsersPodcasts(data.body.items);
        })
  ...
  return (
    <div className="flex-grow h-screen bg-[#121212] overflow-y-scroll scrollbar-hide select-none relative">
      ...
      <section className="ml-8 mr-8">
        <h1 className="mt-6 text-2xl font-bold text-white">Podcasts</h1>
        <div className="mt-4">
